...
def detectarColisionHeroe(listaEnemigos, spritePersonaje):
    for e in range(len(listaEnemigos)-1,-1,-1):
        enemigo = listaEnemigos[e]
        xh = spritePersonaje.rect.left + 47
        yh = spritePersonaje.rect.bottom - 120
        xe, ye, ae, alte = enemigo.rect

        if xh + 10 >= xe and yh >= ye and yh <= ye + alte and xh - 10 <= xe + ae:
            return True
...
def obtenerScores(score):
    ...
    if len(lista2)> 3:
        scoresN = open("scores.txt", "w")
        scoresN.write(str(lista2[0]))
        scoresN.write("\n")
        scoresN.write(str(lista2[1]))
        scoresN.write("\n")
        scoresN.write(str(lista2[2]))
        scoresN.write("\n")
        scoresN.close()
...
def dibujar():
    ...
    imgFondoCreditos = pygame.image.load("CREDSF.png")
    ...
    while not termina:  # Ciclo principal, MIENTRAS la variable termina sea False
        ...
        if estado == JUGANDO:
            ...
            textoTiempo = fuente3.render(str(int(timer2)), 1, BLANCO)
