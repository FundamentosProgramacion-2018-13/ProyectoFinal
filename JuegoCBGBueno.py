...
def dibujarJuego():
    ...
    #   TEXTO
    fuente = pygame.font.SysFont("castellar", 32)
    ...
    while not termina:
        ...
        if estado == Jugar:
            ...
            primerPun = open("Puntaje.txt", "r")
            linea = primerPun.readline()
            puntaje = str(linea)
            primerPun.close()

        elif estado == Derrota:
            ventana.blit(FondoNegro, (0, 0))
            ventana.blit(Gover, (400, 200))

            primerPun = open("Puntaje.txt", "r")
            linea = primerPun.readline()
            puntaje = int(linea[0])
            puntos = checarPuntaje(choque)

            if puntaje < puntos:
                nuevoPun = open("Puntaje.txt", "w")
                nuevoPun.write("%d" % puntos)
                nuevoPun.close()
            primerPun.close()
